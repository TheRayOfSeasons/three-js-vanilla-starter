...
const createLineSpheresGeometry = (radius, vertexCount) => {
  ...
  for (let i = 0; i < vertexCount; i++) {
    ...
    points.push(point);
    spherical.phi = THREE.MathUtils.lerp(Math.PI * 0.5, Math.PI * 1.5, i / vertexCount);
  }
...
export const useLineSpheres = () => {
  (() => {
    ...
    createScene(
      document.getElementById('half-sph-1'),
      (renderer, scene) => {
        scene.add(sphere.object);
        let offset = 0;
        for (let i = sphere.lines.length - 1; i >= 0; i--) {
          sphere.lines[i].rotation.x = -Math.PI * 0.5;
          timeline.add({
            targets: sphere.lines[i].rotation,
            x: 0,
            duration: 1000
          }, offset);
          offset += 50;
  ...
  (() => {
    ...
    createScene(
      document.getElementById('half-sph-2'),
      (renderer, scene) => {
        scene.add(sphere.object);
        let offset = 0;
        for (let i = sphere.lines.length - 1; i >= 0; i--) {
  ...
  (() => {
    ...
    createScene(
      document.getElementById('half-sph-3'),
      (renderer, scene) => {
        scene.add(sphere.object);
        (() => {
          let offset = 0;
  ...
  (() => {
    ...
    createScene(
      document.getElementById('half-sph-4'),
      (renderer, scene) => {
        scene.add(sphere.object);
        let offset = 0;
  ...
  (() => {
    const sphere = getPrefabricatedSphere();
    createScene(
      document.getElementById('half-sph-5'),
      (renderer, scene) => {
        scene.add(sphere.object);
      },
      (time) => {
  ...
  (() => {
    const sphere = getPrefabricatedSphere();
    createScene(
      document.getElementById('half-sph-6'),
      (renderer, scene) => {
        scene.add(sphere.object);
      },
      (time) => {
